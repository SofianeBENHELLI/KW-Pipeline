...
// Orbital preview at `/orb` — clean port of the hi-fi mockup
// (`Orbital Knowledge.zip`). Lives in `apps/web/src/orbital/`; the
// legacy reviewer workbench keeps serving every other path.
const OrbitalApp = lazy(() =>
  import("./orbital/OrbitalApp").then((m) => ({ default: m.OrbitalApp })),
);
...
export default function App() {
  return (
    <Routes>
      ...
      />
      {/* Orbital preview at `/orb*` — clean port of the hi-fi mockup.
          The legacy reviewer workbench (`*`) keeps serving every other
          path. */}
      <Route
        path="/orb/*"
        element={
          <Suspense fallback={<div className="kw-loading">Loading Orbital…</div>}>
            <OrbitalApp />
          </Suspense>
        }
      />
